...
var dialogo = ["This a test. So very long test. So looooong."]
...
func _physics_process(delta):
	if pagina_terminada == true and Input.is_action_just_pressed('interact') and skipable:
		pagina_terminada = false
		pagina_dialogo += 1
		start_dialog()

		timer.one_shot = false
	elif pagina_terminada == false and Input.is_action_just_pressed("skip_dialog") and skipable:
